...
def wait_and_retry_until_status_is_success(max_wait = 20, retry_timeout = 3):
    
    retry_time = time.time() + max_wait
    while time.time() <= retry_time:
        current_status = is_status_success()
        print(f'Status is {current_status}')
        if current_status:
            print('The process is succeeded')
            break
        else:
            time.sleep(retry_timeout)
    else:
        raise Exception (f'The process did not suceed after waiting for {max_wait} seconds.')
    
def wait_and_retry_until_status_is_success2(max_wait = 20, retry_timeout = 3):
    
    retry_time = time.time() + max_wait
    current_status = False
    while time.time() <= retry_time and not current_status:
        current_status = is_status_success()
        print(f'Status is {current_status}')
        time.sleep(retry_timeout)
...
user_selection = input('Select between options 1 and 2: ')
if user_selection == '1':
    wait_and_retry_until_status_is_success()
elif user_selection == '2':
    wait_and_retry_until_status_is_success2()
else: raise Exception ('Enter either 1 or 2')